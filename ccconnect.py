...
def get_tests(self, product, test_cycle, test_run):
    ...
    get_tests_url = "%s/included_test_results?product=%s&cycle=%s&run=%s" % {product, test_cycle, test_run}
...
def set_results(self, tests, environment):
    ...
    test_result_list
    set_results_url = "%s/submit_results?product=%s&cycle=%s&run=%s" % {product, test_cycle, test_run}
